...
import qualified Hasql.CursorTransaction.Private.Queries
                                               as A
...
declareCursor
  :: ByteString -> ByteString -> F.Params params -> params -> C.Transaction ()
...
closeCursor cursorName = C.statement () (A.closeCursor cursorName)

fetchFromCursor
  :: ByteString -> B.BatchSize -> E.Result result -> C.Transaction result
